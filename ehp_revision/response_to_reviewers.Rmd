...
In all these figures, we have used a color scale that is both accessible to readers
with the most common forms of impaired color vision and can be easily distinguished 
when the figure is printed in black and white. 

For these figures, we selected a color palette that was created to meet both these
criteria. [More on viridis]

We have shown an example with one of the figures in the manuscript that uses
this scale. In Figures \@ref(fig:fig5check1) and \@ref(fig:fig5check2) of this
response document, we have reproduced the current version of Figure 5 as it
would appear to someone with three common types of color blindness as well as
when printed using grayscale (i.e., with most black and white printers).

```{r fig5check1, fig.cap="Reproduction of revised version of Figure 5 to check accessibility to readers with two types of colorblindness, deuteranopia (left) and protanopia (right).", out.width="90%", fig.align = "center"}
include_graphics("figures/averageexposureonly_check1.pdf")
```

```{r fig5check2, fig.cap="Reproduction of revised version of Figure 5 to check accessibility to readers with a third type of colorblindness, tritanopia (left) and for printing the figure in grayscale (right).", out.width="90%", fig.align = "center"}
...
As we note in our response to the previous comment, we used a color palette
designed to be perceptible both for those with common types of color blindness
and when printed in grayscale. We have increased the size of the circles in
Figure 3 of the revised manuscript (reproduced in Figure \@ref(fig:fig3) of this
response) so that the colors are clearer, as well as truncated the x-axis as
requested in EHP Request 1. Also, we show in Figures \@ref(fig:fig3check1) and
\@ref(fig:fig3check2) of this response how the revised Figure 3 would look with
three common types of color blindness as well as when printed in black and
white. The gradient is clear when this figure is printed in black and white.

```{r fig3check1, fig.cap="Reproduction of revised version of Figure 3 to check accessibility to readers with two types of colorblindness, deuteranopia (left) and protanopia (right).", out.width="100%"}
include_graphics("figures/windcomparison_check1.pdf")
```

```{r fig3check2, fig.cap="Reproduction of revised version of Figure 3 to check accessibility to readers with a third type of colorblindness, tritanopia (left) and for printing the figure in grayscale (right).", out.width="100%"}
...
As we explain in our response to EHP Request 2, these figures' color palette is
color-blind friendly and also remains perceptible when printed in grayscale.
Figures \@ref(fig:fig5check1) and \@ref(fig:fig5check1) show what Figure 5 would
look like under three common types of color blindness and when printed in black
and white. Figures \@ref(fig:fig6check1) and \@ref(fig:fig6check2) show the same
thing for Figure 6. Therefore, these figures should not require any exemption
from accessibility requirements, as they should satisfy these requirements.

```{r fig6check1, fig.cap="Reproduction of revised version of Figure 6 to check accessibility to readers with two types of colorblindness, deuteranopia (left) and protanopia (right).", fig.show = "hold", out.width="50%"}
include_graphics("figures/jaccard_heatmap_check1.pdf")

include_graphics("figures/jaccard_heatmap_check2.pdf")
```

```{r fig6check2, fig.cap="Reproduction of revised version of Figure 6 to check accessibility to readers with a third type of colorblindness, tritanopia (left) and for printing the figure in grayscale (right).", fig.show = "hold", out.width="50%"}
