...
for_heatmap <- as.matrix(a[ , 2:11])
rownames(for_heatmap) <- a$storm_id

row_dend <- hclust(stats::dist(for_heatmap))
...
ha_row <- HeatmapAnnotation(counties_exposed = counties_exposed$counties_exposed,
                            annotation_legend_param = list(counties_exposed = list(title = "# of\ncounties\nexposed",
                                                                                   title_gp = gpar(fontsize = 9,
                                                                                                   lineheight = 0.7,
                                                                                                   font = 2))),
                            col = list(counties_exposed = colorRamp2(c(200, 300, 400, 500, 600), viridis::viridis(5, option = "C"))),
                            which = "row", width = unit(0.25, "cm"))

heat_map <- Heatmap(for_heatmap, col = viridis(256),
...
pdf(file = "figures/jaccard_heatmap_presentation.pdf", height = 5, width = 6.5)
